...
if (isset($_SESSION["uid"])) {
    ...
    $sql0="SELECT order_id from `checkout_info`";
    $runquery=mysqli_query($con,$sql0);
    if (mysqli_num_rows($runquery) == 0) {
        echo( mysqli_error($con));
        $order_id=1;
    }else if (mysqli_num_rows($runquery) > 0) {
        $sql2="SELECT MAX(order_id) AS max_val from `checkout_info`";
	...
	$sql = "INSERT INTO `checkout_info` 
	(`order_id`,`user_id`,`f_name`, `email`,`address`, 
	`city`, `state`, `cardname`,`expdate`,`prod_count`,`total_amt`) 
	VALUES ($order_id, '$user_id','$f_name','$email', 
    '$address', '$city', '$state','$cardname','$expdate','$total_count','$prod_total')";
